...
const cryptoSha1 = (data) => crypto.createHash('sha1').update(data).digest();

import("../vendor/rust-sha1/binding.mjs").then((wasm) => {
...
while(performance.now() < warmup + 2000) {
	...
	const res = wasm.sha1(data);
	if (Buffer.compare(res, cryptoSha1(data)) !== 0) throw new Error("data validation failed");
}
...
console.log(`sha1 x ${avg1.toFixed(2)} ops/sec ± ${dev1.toFixed(2)} (${(avg4 * avg1 / 1024 / 1024).toFixed(3)} MB/s)`);
