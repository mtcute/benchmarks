...
import("../vendor/mtcute-wasm/index.mjs").then((wasm) => {
const { igeEncrypt, igeDecrypt } = wasm;
...
while(performance.now() < warmup + 2000) {
	const [data, key, iv] = functions.randomizeCrypto(dat, true);
	const c = igeEncrypt(data, key, iv);
	const d = igeDecrypt(c, key, iv);
