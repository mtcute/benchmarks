...
const zlib = require("../build/Release/deflate.node");
...
while(performance.now() < warmup + 2000) {
	...
	const c = zlib.zlib_compress(Buffer.from(data), data.length + 100);
	const d = Buffer.alloc(data.length)
	zlib.zlib_decompress(c, d);
...
while(performance.now() < run + 10000) {
	...
	while(performance.now() < sample + 1000) {
		...
		const c = zlib.zlib_compress(data, data.length + 100);
		if (c === null) throw new Error("zlib_compress failed")
		s1.push(performance.now() - t);
		s3.push(c.length);
		t = performance.now();
		const d = Buffer.alloc(data.length)
		const written = zlib.zlib_decompress(c, d);
