...
// replicating the way it was used in mtcute@0.1.0
const { AesModeOfOperationIge } = require("../node_modules/@mtcute/core/cjs/utils/crypto/common");
const crypto = require("crypto");

function createAesEcb(key) {
	const methodName = `aes-${key.length * 8}-ecb`

	return {
		encrypt(data) {
			const cipher = crypto.createCipheriv(methodName, key, null)
			cipher.setAutoPadding(false)

			return Buffer.concat([cipher.update(data), cipher.final()])
		},
		decrypt(data) {
			const cipher = crypto.createDecipheriv(methodName, key, null)
			cipher.setAutoPadding(false)
...
while(performance.now() < warmup + 2000) {
	...
	const ige = new AesModeOfOperationIge(key, iv, createAesEcb(key));
	const c = ige.encrypt(data);
	const d_ = ige.decrypt(c);
	const d = Buffer.from(d_.buffer, d_.byteOffset, d_.byteLength);
	if(d.toString() !== data.toString()) { console.log(d, data); throw new Error("data validation failed"); }
}
